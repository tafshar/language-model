...
example_batch_predictions = model.forward(input_example_batch, target_example_batch)
...
def train_step(inp, target):
  ...
  with tf.GradientTape() as tape:
    predictions = model.forward(inp, target)
    #forward calc
    loss = tf.reduce_mean(
...
def generate_text(model, start_string):
  ...
  #convert to integer representation
    ...
    input_eval = [SOS_ID] + [sub2idx[s] for s in start_string]
